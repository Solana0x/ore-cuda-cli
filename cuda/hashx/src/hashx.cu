...
int hashx_make(hashx_ctx* ctx, const void* seed, size_t size) {
    ...
    // Initialize the BLAKE2b state with predefined parameters
...
__device__ void hashx_exec(const hashx_ctx* ctx, HASHX_INPUT, void* output) {
    ...
    // Perform finalization steps to reduce bias
    ...
    #else
    uint8_t temp_out_buffer[32];
    if constexpr (HASHX_SIZE > 0) store64(temp_out_buffer + 0, r[0] ^ r[4]);
    if constexpr (HASHX_SIZE > 8) store64(temp_out_buffer + 8, r[1] ^ r[5]);
    if constexpr (HASHX_SIZE > 16) store64(temp_out_buffer + 16, r[2] ^ r[6]);
    if constexpr (HASHX_SIZE > 24) store64(temp_out_buffer + 24, r[3] ^ r[7]);
    memcpy(output, temp_out_buffer, HASHX_SIZE);
    #endif
