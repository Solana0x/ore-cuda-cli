...
// Kernel function to handle program initialization on the GPU
__global__ void initialize_program_kernel(hashx_ctx* ctx, hashx_program* program, siphash_state keys) {
    if (!hashx_program_generate(&keys, program)) {
        return;
    }
    #ifndef HASHX_BLOCK_MODE
    memcpy(&ctx->keys, &keys, 32);
    #else
    memcpy(&ctx->params.salt, &keys, 32);
    #endif
    #ifndef NDEBUG
...
static int initialize_program(hashx_ctx* ctx, hashx_program* program, siphash_state keys[2]) {
    ...
    initialize_program_kernel<<<1, 1>>>(ctx, program, keys[0]);
