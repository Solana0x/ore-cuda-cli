...
__device__ __forceinline__ bool is_mul(instr_type type) {
...
__device__ __forceinline__ const instr_template* select_template(generator_ctx* ctx, instr_type last_instr, int attempt) {
...
__device__ __forceinline__ uint32_t branch_mask(siphash_rng* gen) {
    uint32_t mask = 0;
    int popcnt = 0;
...
__device__ void instr_from_template(const instr_template* tpl, siphash_rng* gen, instruction* instr) {
...
__device__ __forceinline__ bool select_register(int available_regs[8], int regs_count, siphash_rng* gen, int* reg_out) {
    if (regs_count == 0)
        return false;
...
__device__ bool select_destination(const instr_template* tpl, instruction* instr, generator_ctx* ctx, int cycle) {
...
__device__ bool select_source(const instr_template* tpl, instruction* instr, generator_ctx* ctx, int cycle) {
    int available_regs[8];
    int regs_count = 0;
...
__device__ int schedule_instr(const instr_template* tpl, generator_ctx* ctx, bool commit) {
