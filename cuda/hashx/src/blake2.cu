...
static __device__ __forceinline__ void blake2b_increment_counter(blake2b_state* S,
	uint64_t inc) {
	S->t[0] += inc;
	S->t[1] += (S->t[0] < inc);
}

static __device__ __forceinline__ void blake2b_init0(blake2b_state* S) {
	memset(S, 0, sizeof(*S));
...
static void blake2b_compress_4r(blake2b_state* S, const uint8_t* block) {
    // 4-round version, using the same kernel but only running 4 rounds
    blake2b_compress_kernel<<<1, 16>>>(S, block);
    cudaDeviceSynchronize();  // Ensure kernel completion before proceeding
}
