...
__device__ void hashx_program_execute(const hashx_program* program, uint64_t r[8]) {
    ...
    // Unroll the loop with fixed iteration count of 512
    #pragma unroll
    for (int i = 0; i < 512; ++i) {
        const instruction* instr = &program->code[i % program->code_size];
        ...
        switch (instr->opcode) {
            ...
            case INSTR_TARGET:
                ...
                break;
            case INSTR_BRANCH:
                if ((result & instr->imm32) == 0) {
                    i = target;  // Single branch reset to target
                }
                break;
            default:
                UNREACHABLE;
