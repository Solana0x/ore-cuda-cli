...
__device__ uint64_t hash_value(hashx_ctx* hash_func, equix_idx index) {
    char hash[HASHX_SIZE];
    hashx_exec(hash_func, index, hash);
    return load64(hash);
}

// Optimization: Use __forceinline__ to ensure that this small function is inlined, reducing function call overhead.
...
// Optimization: Refactored to reduce redundant memory accesses by storing intermediate results in registers.
