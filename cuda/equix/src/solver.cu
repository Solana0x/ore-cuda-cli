...
// Helper function to build solutions in stage 1
...
// Helper function to build solutions in stage 2
...
// Main function to build final solutions
...
__device__ void solve_stage0(uint64_t* hashes, solver_heap* heap) {
    ...
    for (u32 i = 0; i < INDEX_SPACE; ++i) {
        ...
        if (item_idx >= COARSE_BUCKET_ITEMS) continue;
...
// Stage for hashing initial indices
...
// Stage to solve using intermediate data
__device__ void solve_stage1(solver_heap* heap) {
    ...
    for (u32 bucket_idx = BUCK_START; bucket_idx < BUCK_END; ++bucket_idx) {
        ...
        for (u32 item_idx = 0; item_idx < cpl_buck_size; ++item_idx) {
            ...
            if (fine_item_idx >= FINE_BUCKET_ITEMS) continue;
...
// Second stage to refine the solving process
...
// Final stage to attempt solving the problem
...
// Kernel to solve all stages in one go
